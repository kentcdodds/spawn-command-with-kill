...
describe('crossSpawnWithKill', () => {
  ...
  beforeEach(() => {
    jest.mock('spawn-command', () => jest.fn(() => ({pid: 32})))
  ...
  it('can be killed', () => {
    ...
    expect(spawnCommand).toBeCalledWith('cmd', ['arg1'], {options: 1})
  ...
  it('has a win32 kill function when the platform is win32', () => {
    ...
    expect(spawnCommand).toBeCalled()
    expect(spawnCommand).toBeCalledWith('taskkill', ['/F', '/T', '/PID', 32])
  })
